...
**DEPRICATION:**
With the addition of the :func:pychm.io.pdb.get_molFromCRD function,
...
import os
import itertools
from pychm.const import alphanum, alpha
from pychm.tools import Property, expandPath, paragraphs, lowerKeys
...
##################
    ...
    def keys(self):
        ...
        object contains. In this case, CRD files only have one model.
        """
        return [ 'model0' ]
    ...
    def _build_crd(self):
        ...
        if self.inFormat == 'shortcard':
            iterator = ( Atom(text=line, informat='shortcard', index=i,
                        autofix=self._autoFix) for i, line in enumerate(self.crd) if len(line) == 71 )
        elif self.inFormat == 'longcard':
            iterator = ( Atom(text=line, informat='longcard', index=i,
                        autofix=self._autoFix) for i, line in enumerate(self.crd) if len(line) == 141 )

        self._models[0] = Mol(iterable=iterator, name='crdmol', autofix=False)
